...
function show_packs_with_archive($user_id) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        if (isset($_POST['archive_pack_id'])) {
            ...
            $update_query = "UPDATE `paczki` SET `is_archived` = 1 WHERE `id_paczki` = '$pack_id' AND FIND_IN_SET('$user_id', `user_id`)";
            mysqli_query($conn, $update_query);
        }

        if (isset($_POST['unarchive_pack_id'])) {
            $pack_id = intval($_POST['unarchive_pack_id']);
            $update_query = "UPDATE `paczki` SET `is_archived` = 0 WHERE `id_paczki` = '$pack_id' AND FIND_IN_SET('$user_id', `user_id`)";
            mysqli_query($conn, $update_query);
        }
    }

    // Pobieranie paczek użytkownika
    $query = "SELECT `id_paczki`, `nazwa_paczki`, `is_archived` FROM `paczki` WHERE FIND_IN_SET('$user_id', `user_id`)";

...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'create_pack') {
    ...
    $query = "INSERT INTO paczki (id_users, name) VALUES ('$userId', '$packName')";
...
// Pobieranie losowego słowa z wybranej paczki
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'check_answer') {
    ...
    if (strtolower($userAnswer) === strtolower($correctAnswer)) {
        $message = "Brawo! Dobra odpowiedź!";
    } else {
        $message = "Źle! Poprawna odpowiedź to: $correctAnswer.";
    }
}
...
if (isset($_SESSION['user_id'])) {
    $userId = $_SESSION['user_id'];
    $query = "SELECT * FROM paczki WHERE id_users = '$userId'";
    $result = mysqli_query($conn, $query);
